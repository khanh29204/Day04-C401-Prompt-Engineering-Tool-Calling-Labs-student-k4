"""Pydantic response models for the CGV mobile API.

Ported verbatim (aside from formatting) from https://github.com/t-rekttt/cgv-mcp
(`src/cgv_mcp/models.py`). Kept as-is so the internal tools validate the same
shapes the original MCP server did.

If CGV changes a field, validation raises ValidationError and the calling tool
returns it through `tools._shared.err()` instead of crashing the agent loop.
"""
